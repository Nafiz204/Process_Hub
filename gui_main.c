...
static void on_fg_dialog_response(GtkDialog *dialog, gint response_id, gpointer data) {
    ...
    if (response_id == 2) { // Kill button
        kill(pid, SIGKILL);
    }
    // Just hide it; the main loop will catch the visibility change and destroy it safely
...
static void run_foreground_wait(AppWidgets *widgets, pid_t pid, const char *cmd) {
    ...
    // Safety: set the pointer to NULL if the widget is destroyed externally
    ...
    while (dialog_ptr != NULL) {
        ...
        // Break if hidden (user clicked button or 'X')
...
static void on_run_command(GtkWidget *widget, gpointer data) {
    ...
    if (is_builtin(args[0])) {
        ...
        pid_t pid = execute_external(args, background);
        if (!background && pid > 0) {
...
int main(int argc, char *argv[]) {
    ...
    gtk_entry_set_placeholder_text(GTK_ENTRY(entry_run), "e.g. sleep 10 or sleep 10 &");
    ...
    widgets->sys_store = gtk_list_store_new(SYS_NUM_COLS, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_DOUBLE, G_TYPE_LONG);
    ...
    const char *sys_headers[] = {"PID", "Status", "Command", "CPU%", "Mem(kB)"};
    for (int i = 0; i < SYS_NUM_COLS; i++) {
    ...
    widgets->job_store = gtk_list_store_new(JOB_NUM_COLS, G_TYPE_INT, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING);
